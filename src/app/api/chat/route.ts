  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Hugging Face API key is required but not set in environment variables",
        },
        { status: 401 } // Using 401 Unauthorized instead of 503 for missing credentials
      );
    }
    if (!req.body) {
      return NextResponse.json(
    ...
    try {
      ...
      if (cleanedText.includes("Assistant:")) {
        cleanedText =
          cleanedText.split("Assistant:").pop()?.trim() || cleanedText;
      }
